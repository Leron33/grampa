...
M = load_off(strcat(path_kids,track, 'kid', num2str(i), '.off'));
N = load_off(strcat(path_kids,track, 'kid', num2str(j), '.off'));
...
%% GRAMPA
tic;
X = sparse(matching_robust_spectral(full(adj1), full(adj2), 1));
P_sp = greedy_match(X);   
run_time_grampa=toc;
if n1<=n2
